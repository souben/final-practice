...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		// implement
		Stream<String> stream1 = list1.stream().filter(x-> !list2.contains(x));
		Stream<String> stream2 = list2.stream().filter(x-> !list1.contains(x));

		List<String> inJustOne = Stream.concat(stream1, stream2).collect(Collectors.toList());
		return inJustOne ;
	}
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		...
		List<String> allFairfieldCustomers = list.stream()
				.filter(x-> x.getCity().equals("Fairfield"))
				.map(x->x.getName()).sorted()
				.collect(Collectors.toList());
		
		return allFairfieldCustomers;
	}
